...
function buildRuleDraft(payload) {
  ...
  const mkQ = (number, type, axis, instruction, hint, resources, interaction, scoring, diagnostics) => ({
    ...
    lrs: { verb: 'answered', objectId: `quiz_${storyId}_v3_Q${String(number).padStart(2, '0')}_${axis}`, resultFields: ['score_raw', 'hint_used', 'response_time_sec', 'selected_key'] }
  });
  return {
...
const quiz=${data};
var current=0;var answers={};var checked={};var rtStart={};var rtData={};var skData={};
...
function check(){var q=quiz.questions[current];var id=q.qId||q.number;checked[id]=scoreCurrent();rtData[id]=Math.round((Date.now()-(rtStart[id]||Date.now()))/100)/10;skData[id]=(answers[id]&&answers[id].selected)||null;render();}
